...
def print_image(
    ...
    **kwargs,
    ...
    if backend == "climage":
        ...
        if not any(
            x in kwargs
            for x in ("is_truecolor", "is_256color", "is_16color", "is_8color")
        ):
...
def display_image(
    image, *, width=None, height=None, background=255, output_format="jpeg"
):
    ...
    if _in_notebook():
        ...
        if width is None and height is None:
            width, height = (
                f"{min(image.size[0], 1280)}px",
                f"{min(image.size[1], 720)}px",
            )
