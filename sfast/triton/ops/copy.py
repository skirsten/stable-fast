...
def copy_2d_kernel(
    ...
    A = input_ptr + batch_stride_inp * pid_batch + rm * stride_inp_0
    B = output_ptr + batch_stride_out * pid_batch + rm * stride_out_0
    mask = rm < size_inp_0
...
def copy_3d_kernel(
    ...
    A = (
        input_ptr
        + batch_stride_inp * pid_batch
        + (rm[:, None] * stride_inp_0 + rn[None, :] * stride_inp_1)
    )
    B = (
        output_ptr
        + batch_stride_out * pid_batch
...
def copy_4d_kernel(
    ...
    A = (
        ...
        + batch_stride_inp * pid_batch
        + (
            rm[:, None, None] * stride_inp_0
            + rn[None, :, None] * stride_inp_1
            + rk[None, None, :] * stride_inp_2
        )
    )
    B = (
        output_ptr
        + batch_stride_out * pid_batch
        + (
            rm[:, None, None] * stride_out_0
            + rn[None, :, None] * stride_out_1
            + rk[None, None, :] * stride_out_2
        )
    )
    mask = (
        (rm < size_inp_0)[:, None, None]
        & (rn < size_inp_1)[None, :, None]
...
def copy(dst, src):
    ...
    if ndim in (1, 2):
        ...
        copy_2d_kernel[grid](dst, src, bsz, sz0, bss, ss0, bsd, sd0)
    elif ndim == 3:
        ...
        def grid(meta):
            return (
                triton.cdiv(sz0, meta["BLOCK_M"]) * triton.cdiv(sz1, meta["BLOCK_N"]),
                bs,
            )

        copy_3d_kernel[grid](dst, src, bs, sz0, sz1, bss, ss0, ss1, bsd, sd0, sd1)
    elif ndim == 4:
        ...
        def grid(meta):
            return (
                triton.cdiv(sz0, meta["BLOCK_M"])
                * triton.cdiv(sz1, meta["BLOCK_N"])
                * triton.cdiv(sz2, meta["BLOCK_K"]),
                bs,
            )

        copy_4d_kernel[grid](
            dst, src, bs, sz0, sz1, sz2, bss, ss0, ss1, ss2, bsd, sd0, sd1, sd2
        )
    else:
